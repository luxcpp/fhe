...
//
// Batch Vectorization Optimization (2025-12):
// - Process entire [batch, N] tensor in single kernel launch
// - No sequential loops over batch dimension
// - Precomputed index patterns amortized across NTT calls
// - Unified twiddle buffer eliminates per-stage allocation overhead
// - Fused diagonal twiddle in four-step NTT
// - Target: 2-3x additional speedup on top of 6.48x baseline
//
...
// =============================================================================
// Precomputed Index Cache for Batch NTT
// =============================================================================
// Amortizes index computation across all NTT calls for given N.
// Key optimization: Indices are computed once at construction, reused forever.
...
public:
    ...
    static NTTIndexCache& get_forward(uint32_t N, uint32_t log_N) {
        static std::unordered_map<uint32_t, NTTIndexCache> cache;
        auto it = cache.find(N);
        if (it == cache.end()) {
            cache.emplace(N, NTTIndexCache(N, log_N, true));
        }
        return cache.at(N);
    }

    static NTTIndexCache& get_inverse(uint32_t N, uint32_t log_N) {
        static std::unordered_map<uint32_t, NTTIndexCache> cache;
        auto it = cache.find(N);
        if (it == cache.end()) {
            cache.emplace(N, NTTIndexCache(N, log_N, false));
        }
        return cache.at(N);
    }

    const StageIndices& stage(uint32_t s) const { return stages_[s]; }

private:
    std::vector<StageIndices> stages_;

    NTTIndexCache(uint32_t N, uint32_t log_N, bool forward) {
        stages_.reserve(log_N);
        ...
        for (uint32_t s = 0; s < log_N; ++s) {
            ...
            if (forward) {
                ...
                for (uint32_t i = 0; i < m; ++i) {
                    for (uint32_t j = 0; j < t; ++j) {
                        ...
                        hi[idx] = static_cast<int32_t>(idx_lo + t);
            ...
            // Create arrays directly (no default constructor for mx::array)
            mx::array lo_arr(lo.data(), {half_N}, mx::int32);
            mx::array hi_arr(hi.data(), {half_N}, mx::int32);
            mx::array tw_arr(tw.data(), {half_N}, mx::int32);
            mx::eval(lo_arr);
            mx::eval(hi_arr);
            mx::eval(tw_arr);
            stages_.push_back(StageIndices{std::move(lo_arr), std::move(hi_arr), std::move(tw_arr)});
        }
    }
};

// =============================================================================
// OPTIMIZED: Batch-Vectorized Forward Stage
// =============================================================================
// Key change: Process ALL batches in single kernel launch via axis=1 operations.
// Old code had sequential loop over batch dimension - this eliminates it.

inline void NTTMetalDispatcher::forward_stage_optimized(mx::array& data, uint32_t stage) {
    // Use cached indices (amortized across all calls)
    const auto& idx_cache = NTTIndexCache::get_forward(params_.N, params_.log_N);
    const auto& stage_idx = idx_cache.stage(stage);
    const auto& lo_idx = stage_idx.lo;
    const auto& hi_idx = stage_idx.hi;
    const auto& tw_idx = stage_idx.tw;

    // Use stage-indexed twiddles
    const auto& stage_tw = fwd_twiddles_.stage_tw[stage];

    uint64_t Q = params_.Q;
    auto Q_arr = mx::array(static_cast<int64_t>(Q));

    // VECTORIZED: Gather along coefficient axis (1) for ALL batches at once
    // Shape: data [batch, N] -> lo_vals [batch, N/2]
    auto lo_vals = mx::take(data, lo_idx, 1);
    auto hi_vals = mx::take(data, hi_idx, 1);

    // Twiddles: [stage_size] -> automatically broadcast to [batch, N/2]
    ...
    // NO mx::eval() - let MLX JIT fuse consecutive stages
}
...
// =============================================================================
// OPTIMIZED: Batch-Vectorized Inverse Stage
// =============================================================================
// Same optimization as forward: axis=1 operations process all batches at once.

inline void NTTMetalDispatcher::inverse_stage_optimized(mx::array& data, uint32_t stage) {
    // ULTRA-OPTIMIZED: Use reshape+slice instead of scatter/gather
    // Gentleman-Sande inverse NTT butterfly structure:
    //   Stage s: m = N/2^(s+1) groups, each group has 2*t elements
    //   where t = 2^s
    ...
    // Gentleman-Sande structure (opposite of Cooley-Tukey):
    //   m = N / 2^(s+1) = number of groups
    //   t = 2^s = elements per half-group
    uint32_t m = N >> (stage + 1);
    uint32_t t = 1u << stage;

    // Get twiddles for this stage
    const auto& stage_tw = inv_twiddles_.stage_tw[stage];

    // Reshape data to expose butterfly pairs:
    // [batch, N] -> [batch, m, 2, t]
    auto reshaped = mx::reshape(data, {batch, static_cast<int>(m), 2, static_cast<int>(t)});

    // Split into lo and hi halves: each [batch, m, t]
    auto lo_vals = mx::slice(reshaped, {0, 0, 0, 0}, {batch, static_cast<int>(m), 1, static_cast<int>(t)});
    auto hi_vals = mx::slice(reshaped, {0, 0, 1, 0}, {batch, static_cast<int>(m), 2, static_cast<int>(t)});

    lo_vals = mx::squeeze(lo_vals, 2);
    hi_vals = mx::squeeze(hi_vals, 2);

    // Broadcast twiddles: [m] -> [1, m, 1] for broadcast to [batch, m, t]
    auto tw_vals = mx::reshape(stage_tw, {1, static_cast<int>(m), 1});

    // GS butterfly: new_lo = (lo + hi), new_hi = (lo - hi) * tw, all mod Q
    auto sum = mx::remainder(mx::add(lo_vals, hi_vals), Q_arr);
    auto diff = mx::subtract(lo_vals, hi_vals);
    diff = mx::remainder(mx::add(diff, Q_arr), Q_arr);  // Ensure non-negative
    auto new_hi = mx::remainder(mx::multiply(diff, tw_vals), Q_arr);

    // Expand dims back: [batch, m, t] -> [batch, m, 1, t]
    sum = mx::expand_dims(sum, 2);
    new_hi = mx::expand_dims(new_hi, 2);

    // Concatenate along the pair dimension: [batch, m, 2, t]
    auto result = mx::concatenate({sum, new_hi}, 2);

    // Reshape back to [batch, N]
    data = mx::reshape(result, {batch, N});
    // NO mx::eval() - let MLX JIT fuse stages
}
...
inline void NTTMetalDispatcher::forward(mx::array& data) {
    ...
    // OPTIMIZATION: Only eval input once, let MLX batch all stages
    // This avoids GPU sync after each stage (was 10x overhead for small N)
    mx::eval(data);
    ...
    for (uint32_t s = 0; s < params_.log_N; ++s) {
        ...
        // NO mx::eval() here - let MLX JIT fuse operations
    }

    // Single sync at the end - MLX will fuse all stages into optimized kernel
    mx::eval(data);
...
inline void NTTMetalDispatcher::inverse(mx::array& data) {
    ...
    // OPTIMIZATION: Only eval input once
    mx::eval(data);
    ...
    // Scale by N^{-1} (vectorized across all batches) - fused with stages
