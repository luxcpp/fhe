...
kernel void signal_prefetch_complete(
    ...
    atomic_store_explicit(completion_flag, expected_value + 1, memory_order_release);
}
...
kernel void wait_prefetch_complete(
    ...
    while (atomic_load_explicit(completion_flag, memory_order_acquire) < expected_value) {
        // Spin
    }
}
