...
// =============================================================================
// Fused Blind Rotation Metal Kernel Dispatcher
// =============================================================================
//
// Dispatches the fused blind rotation kernel that processes all n iterations
// in a single GPU launch, eliminating 512 kernel launches per bootstrap.
//
// Performance:
//   Before: 512 kernel launches per bootstrap
//   After:  1 kernel launch per bootstrap
//   Expected speedup: 5-10x

// Kernel parameters struct (must match Metal shader)
...
// Metal kernel source for fused blind rotation
inline NSString* get_blind_rotate_kernel_source() {
    // Load from external file or embed inline
    // For now, we embed a minimal version; full version is in blind_rotate_fused.metal
    return @R"METAL(
#include <metal_stdlib>
using namespace metal;
...
inline uint64_t barrett_mul(uint64_t a, uint64_t omega, uint64_t Q, uint64_t precon) {
...
inline void ct_butterfly(threadgroup uint64_t* data, uint32_t idx_lo, uint32_t idx_hi,
...
inline void gs_butterfly(threadgroup uint64_t* data, uint32_t idx_lo, uint32_t idx_hi,
                         uint64_t omega, uint64_t precon, uint64_t Q) {
    uint64_t lo_val = data[idx_lo];
    uint64_t hi_val = data[idx_hi];
    uint64_t sum = mod_add(lo_val, hi_val, Q);
...
kernel void blind_rotate_fused(
    ...
    for (uint32_t j = 0; j < n; ++j) {
        ...
        for (uint32_t i = local_id; i < N; i += num_threads) {
            ...
            while (src_idx < 0) { src_idx += N; negate = !negate; }
            while (src_idx >= int32_t(N)) { src_idx -= N; negate = !negate; }
            rot_c0[i] = negate ? mod_sub(0, acc_c0[src_idx], Q) : acc_c0[src_idx];
            rot_c1[i] = negate ? mod_sub(0, acc_c1[src_idx], Q) : acc_c1[src_idx];
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
...
public:
    ...
    FusedBlindRotate(const Config& cfg);
    ~FusedBlindRotate();

    // Execute fused blind rotation
    // lweBatch: [batch, n+1] - Input LWE ciphertexts
    // bsk: [n, 2, L, 2, N] - Bootstrap key
    // testPoly: [N] - Test polynomial
    // Returns: [batch, 2, N] - Output RLWE ciphertexts
    void execute(int64_t* acc_out,
...
#ifdef WITH_MLX
    mx::array execute(const mx::array& lweBatch,
                      const mx::array& bsk,
    ...
    struct Metrics {
        uint64_t kernel_launches = 0;
        double total_time_ms = 0;
        double avg_time_per_bootstrap_ms = 0;
    };
...
private:
    ...
    id<MTLBuffer> twiddles_buffer_ = nil;
    id<MTLBuffer> tw_precon_buffer_ = nil;
    id<MTLBuffer> inv_tw_buffer_ = nil;
    id<MTLBuffer> inv_precon_buffer_ = nil;
...
inline FusedBlindRotate::FusedBlindRotate(const Config& cfg) : cfg_(cfg) {
    // Initialize params
    params_.N = cfg.N;
    params_.n = cfg.n;
    params_.L = cfg.L;
    params_.Q = cfg.Q;
    params_.base_log = cfg.baseLog;
    params_.base_mask = (1ULL << cfg.baseLog) - 1;

    // Compute log_N
    params_.log_N = 0;
    while ((1u << params_.log_N) < cfg.N) ++params_.log_N;

    // Compute Barrett constant
    params_.mu = static_cast<uint64_t>((__uint128_t)1 << 64) / cfg.Q;
    ...
    params_.N_inv = mod_inv(cfg.N, cfg.Q);
    params_.N_inv_precon = static_cast<uint64_t>(((__uint128_t)params_.N_inv << 64) / cfg.Q);
...
inline FusedBlindRotate::~FusedBlindRotate() {
    // ARC handles cleanup
}

inline void FusedBlindRotate::compile_kernel() {
    NSError* error = nil;

    id<MTLLibrary> library = [device_ newLibraryWithSource:get_blind_rotate_kernel_source()
                                                   options:nil
                                                     error:&error];
    if (!library) {
        NSLog(@"Failed to compile blind rotate kernel: %@", error);
        available_ = false;
        return;
    }

    id<MTLFunction> func = [library newFunctionWithName:@"blind_rotate_fused"];
    if (!func) {
        available_ = false;
        return;
    }

    pipeline_ = [device_ newComputePipelineStateWithFunction:func error:&error];
    available_ = (pipeline_ != nil);
}

inline void FusedBlindRotate::init_twiddles() {
    uint32_t N = cfg_.N;
    uint64_t Q = cfg_.Q;
    ...
    std::vector<uint64_t> tw(N), tw_precon(N);
    std::vector<uint64_t> inv_tw(N), inv_tw_precon(N);
    ...
    for (uint32_t m = 1; m < N; m <<= 1) {
        ...
        for (uint32_t i = 0; i < m; ++i) {
            ...
            inv_tw_precon[m + i] = static_cast<uint64_t>(((__uint128_t)inv_tw[m + i] << 64) / Q);
        }
    }
    tw[0] = 1;
    ...
    twiddles_buffer_ = [device_ newBufferWithBytes:tw.data()
                                            length:tw_bytes
                                           options:MTLResourceStorageModeShared];
    tw_precon_buffer_ = [device_ newBufferWithBytes:tw_precon.data()
                                             length:tw_bytes
                                            options:MTLResourceStorageModeShared];
    inv_tw_buffer_ = [device_ newBufferWithBytes:inv_tw.data()
                                          length:tw_bytes
                                         options:MTLResourceStorageModeShared];
    inv_precon_buffer_ = [device_ newBufferWithBytes:inv_tw_precon.data()
                                              length:tw_bytes
                                             options:MTLResourceStorageModeShared];
}

inline void FusedBlindRotate::execute(int64_t* acc_out,
                                       const int64_t* lwe_in,
                                       const int64_t* bsk,
                                       const int64_t* test_poly,
                                       uint32_t batch_size) {
    ...
    // Update params
    BlindRotateParams local_params = params_;
    local_params.batch_size = batch_size;

    // Create buffers
    size_t acc_bytes = batch_size * 2 * N * sizeof(int64_t);
    size_t lwe_bytes = batch_size * (n + 1) * sizeof(int64_t);
    size_t bsk_bytes = n * 2 * L * 2 * N * sizeof(int64_t);
    size_t test_bytes = N * sizeof(int64_t);

    id<MTLBuffer> acc_buffer = [device_ newBufferWithBytesNoCopy:(void*)acc_out
                                                          length:acc_bytes
                                                         options:MTLResourceStorageModeShared
                                                     deallocator:nil];
    id<MTLBuffer> lwe_buffer = [device_ newBufferWithBytes:lwe_in
                                                    length:lwe_bytes
                                                   options:MTLResourceStorageModeShared];
    id<MTLBuffer> bsk_buffer = [device_ newBufferWithBytes:bsk
                                                    length:bsk_bytes
                                                   options:MTLResourceStorageModeShared];
    id<MTLBuffer> test_buffer = [device_ newBufferWithBytes:test_poly
                                                     length:test_bytes
                                                    options:MTLResourceStorageModeShared];
    ...
    [encoder setBuffer:acc_buffer offset:0 atIndex:0];
    [encoder setBuffer:lwe_buffer offset:0 atIndex:1];
    [encoder setBuffer:bsk_buffer offset:0 atIndex:2];
    [encoder setBuffer:test_buffer offset:0 atIndex:3];
    [encoder setBuffer:twiddles_buffer_ offset:0 atIndex:4];
    [encoder setBuffer:tw_precon_buffer_ offset:0 atIndex:5];
    [encoder setBuffer:inv_tw_buffer_ offset:0 atIndex:6];
    [encoder setBuffer:inv_precon_buffer_ offset:0 atIndex:7];
    [encoder setBytes:&local_params length:sizeof(local_params) atIndex:8];

    // Shared memory: 5 * N * sizeof(uint64_t) = 40KB for N=1024
    NSUInteger shared_mem = 5 * N * sizeof(uint64_t);

    // Threadgroup size: N/2 threads (for butterfly operations)
    uint32_t tg_size = std::min(N / 2, 512u);
    ...
    metrics_.kernel_launches++;
    metrics_.total_time_ms += elapsed;
    metrics_.avg_time_per_bootstrap_ms = metrics_.total_time_ms / metrics_.kernel_launches / batch_size;
}

#ifdef WITH_MLX
inline mx::array FusedBlindRotate::execute(const mx::array& lweBatch,
                                            const mx::array& bsk,
                                            const mx::array& testPoly) {
    mx::eval(lweBatch, bsk, testPoly);

    auto lwe_shape = lweBatch.shape();
    int batch_size = lwe_shape[0];
    int N = static_cast<int>(cfg_.N);

    // Allocate output
    std::vector<int64_t> acc_data(batch_size * 2 * N);

    execute(acc_data.data(),
            lweBatch.data<int64_t>(),
            bsk.data<int64_t>(),
            testPoly.data<int64_t>(),
            batch_size);

    return mx::array(acc_data.data(), {batch_size, 2, N}, mx::int64);
}
#endif
...
// Non-Apple stub for FusedBlindRotate
class FusedBlindRotate {
public:
    struct Config {
        uint32_t N = 1024;
    ...
    FusedBlindRotate(const Config&) {}
    ~FusedBlindRotate() = default;

    void execute(int64_t*, const int64_t*, const int64_t*, const int64_t*, uint32_t) {}
