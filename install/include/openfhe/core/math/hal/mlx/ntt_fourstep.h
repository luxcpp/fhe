...
#ifndef LBCRYPTO_MATH_HAL_MLX_NTT_FOURSTEP_H
#define LBCRYPTO_MATH_HAL_MLX_NTT_FOURSTEP_H
...
inline void NTTFourStep::row_ntt(mx::array& matrix, bool inverse) {
    ...
    // Process all rows in parallel by vectorizing across the column dimension
    // matrix: [n1, n2] -> transpose to [n2, n1] -> batch NTT -> transpose back

    // Transpose to [n2, n1] for efficient row processing
    auto transposed = mx::transpose(matrix, {1, 0});
    mx::eval(transposed);

    // Each row is now contiguous - perform stage-by-stage NTT
    for (uint32_t stage = 0; stage < log_n1; ++stage) {
        uint32_t m = 1u << stage;
        uint32_t t = n1 >> (stage + 1);
        int half_n = static_cast<int>(n1 / 2);

        // Build butterfly indices
        std::vector<int32_t> lo_indices(half_n);
        std::vector<int32_t> hi_indices(half_n);
        std::vector<int32_t> tw_indices(half_n);
        ...
        auto lo_vals = mx::take(transposed, lo_idx, 1);  // [n2, half_n]
        auto hi_vals = mx::take(transposed, hi_idx, 1);  // [n2, half_n]
        auto tw_vals = mx::take(tw, tw_idx, 0);          // [half_n]

        // Broadcast twiddles across batch dimension
        auto Q_arr = mx::array(static_cast<int64_t>(Q_));
...
// -----------------------------------------------------------------------------
// Column NTT: n1 parallel NTTs of size n2
// -----------------------------------------------------------------------------
// After transpose, columns become rows, so we can reuse row_ntt logic.
...
inline void NTTFourStep::col_ntt(mx::array& matrix, bool inverse) {
    ...
    // matrix is now [n2, n1] after transpose
    // Process columns (which are now rows in this transposed view)
    ...
    for (uint32_t stage = 0; stage < log_n2; ++stage) {
        ...
        // Process all columns (rows in transposed view)
        auto lo_vals = mx::take(matrix, lo_idx, 0);  // [half_n, n1]
        auto hi_vals = mx::take(matrix, hi_idx, 0);  // [half_n, n1]
        auto tw_vals = mx::take(tw, tw_idx, 0);      // [half_n]

        // Broadcast twiddles
        tw_vals = mx::expand_dims(tw_vals, 1);  // [half_n, 1]
...
inline void NTTFourStep::forward_batch(mx::array& data) {
    ...
    for (int b = 0; b < batch; ++b) {
        auto poly = mx::slice(data, {b, 0}, {b + 1, N_in});
        poly = mx::reshape(poly, {N_in});

        forward(poly);

        data = mx::scatter(data, mx::array({b}), mx::reshape(poly, {1, N_in}), 0);
    }
    mx::eval(data);
}
...
#endif // LBCRYPTO_MATH_HAL_MLX_NTT_FOURSTEP_H
